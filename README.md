...
- 날짜가 꼭 필요하지 않은 Todo는 주간 task로 분리해 이번 주 안에 처리할 수 있어야 합니다.
...
- Month의 지난 날짜는 일자 옆에 완료율 상태를 작은 기호로 표시합니다. 완료일은 초록 체크, 70% 이상은 초록 점, 30% 이상은 파란 점, 30% 미만은 빨간 점입니다.
- Todo는 `Day` 또는 `Week` 타입으로 만들 수 있습니다.
- `Day` Todo는 날짜별 셀에 표시되고, `Week` Todo는 Week 화면 하단의 Week Tasks 영역에 표시됩니다.
...
- Routines 화면에서 기존 루틴을 편집할 수 있습니다.
- Completed 화면은 선택한 월 기준으로 완료 항목 목록과 태그별 통계를 표시합니다.
- macOS 메뉴와 메뉴바 extra에서 간단한 Pomodoro 타이머를 사용할 수 있습니다.
- 앱 창은 기본 1280x820 크기로 열리고, 1120x720보다 작게 줄어들지 않습니다.
- 왼쪽 사이드바 하단에는 앱 버전만 작고 흐린 텍스트로 표시됩니다.
...
버전을 올리지 않고 현재 `VERSION` 값으로만 패키징해야 할 때는 다음처럼 실행합니다.

```sh
VOIDX_SKIP_VERSION_BUMP=1 ./scripts/run-app.sh
```
...
현재 앱 버전은 `VERSION` 파일에서 관리합니다. 초기 버전은 `1.0`이며, `./scripts/run-app.sh` 또는 `./scripts/install-widget-app.sh`가 성공적으로 패키징/설치될 때마다 `1.1`, `1.2`처럼 `0.1`씩 증가합니다.

빌드 번호는 버전의 `major * 10 + minor` 값으로 계산합니다. 예를 들어 `1.0`은 build `10`, `1.1`은 build `11`입니다.
...
CloudKit 전환 준비는 `docs/CLOUDKIT-PREP.md`를 기준으로 진행합니다. 현재 앱은 로컬 JSON을 기본 저장소로 유지하고, 이후 CloudKit sync layer를 추가하는 방향입니다.

## 개발 지침

### 1. MVP 범위를 지킵니다

현재 우선순위는 로컬 개인 Todo 앱의 완성도입니다. CloudKit sync는 로컬 저장 안정성을 유지한 채 단계적으로 붙이고, 알림, 로그인, 협업, 태그 고도화, 외부 캘린더 연동은 핵심 흐름이 안정된 뒤 검토합니다.

### 2. 화면 목적을 분명히 유지합니다

- Today: 빠른 확인, 빠른 추가, 빠른 완료
- Week: 날짜별 업무 분포와 이번 주 안에 처리할 task를 함께 파악
- Month: 한 달 전체의 밀도 파악
...
Week 화면에서는 특정 날짜에 걸린 일과 주간 단위로 처리하면 되는 일을 섞지 않습니다. 날짜가 중요하면 `Day`, 이번 주 안에 끝내면 충분하면 `Week` 타입을 사용합니다.
...
- Month의 완료율 표시는 지난 날짜에만 노출하고, 일자 텍스트 옆의 작은 기호로 제한해 캘린더 밀도를 해치지 않습니다.
...
버전 증가가 필요 없는 검증 빌드는 `VOIDX_SKIP_VERSION_BUMP=1 ./scripts/run-app.sh`를 사용합니다.
...
메뉴바 기능은 Todo 관리 흐름을 보조하는 수준으로 유지합니다. Pomodoro처럼 상태가 작은 도구는 앱 전역 상태로 두되, 복잡한 알림 권한이나 기록 저장이 필요해지면 별도 설계를 먼저 둡니다.
...
필요해지면 별도 설계 문서로 범위와 저장 모델을 먼저 정한 뒤 구현합니다. CloudKit 관련 범위는 `docs/CLOUDKIT-PREP.md`에서 관리합니다.
